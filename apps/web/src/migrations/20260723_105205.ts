...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   CREATE TYPE "public"."enum_users_role" AS ENUM('visitor', 'retail', 'wholesale', 'staff', 'admin');
  CREATE TYPE "public"."enum_users_customer_type" AS ENUM('RETAIL', 'WHOLESALE');
  CREATE TABLE IF NOT EXISTS "users" (
  	"id" serial PRIMARY KEY NOT NULL,
  	"role" "enum_users_role" DEFAULT 'retail' NOT NULL,
  	"customer_type" "enum_users_customer_type" DEFAULT 'RETAIL' NOT NULL,
  ...
  CREATE TABLE IF NOT EXISTS "products" (
  	...
  	"active" boolean DEFAULT true,
  	"featured" boolean DEFAULT false,
  	"is_solistica" boolean DEFAULT false,
  	"sort_order" numeric DEFAULT 0,
  ...
  CREATE TABLE IF NOT EXISTS "product_attributes_values" (
  ...
  DO $$ BEGIN
   ALTER TABLE "product_attributes_values" ADD CONSTRAINT "product_attributes_values_parent_id_fk" FOREIGN KEY ("_parent_id") REFERENCES "public"."product_attributes"("id") ON DELETE cascade ON UPDATE no action;
  EXCEPTION
   WHEN duplicate_object THEN null;
  END $$;
  ...
  CREATE UNIQUE INDEX IF NOT EXISTS "product_tags_slug_idx" ON "product_tags" USING btree ("slug");
  CREATE INDEX IF NOT EXISTS "product_tags_updated_at_idx" ON "product_tags" USING btree ("updated_at");
  CREATE INDEX IF NOT EXISTS "product_tags_created_at_idx" ON "product_tags" USING btree ("created_at");
  ...
  CREATE INDEX IF NOT EXISTS "products_rels_product_tags_id_idx" ON "products_rels" USING btree ("product_tags_id");
  CREATE INDEX IF NOT EXISTS "products_rels_product_attributes_id_idx" ON "products_rels" USING btree ("product_attributes_id");
  CREATE INDEX IF NOT EXISTS "products_rels_product_images_id_idx" ON "products_rels" USING btree ("product_images_id");
  CREATE INDEX IF NOT EXISTS "product_attributes_values_order_idx" ON "product_attributes_values" USING btree ("_order");
  CREATE INDEX IF NOT EXISTS "product_attributes_values_parent_id_idx" ON "product_attributes_values" USING btree ("_parent_id");
  CREATE UNIQUE INDEX IF NOT EXISTS "product_attributes_slug_idx" ON "product_attributes" USING btree ("slug");
  CREATE INDEX IF NOT EXISTS "product_attributes_updated_at_idx" ON "product_attributes" USING btree ("updated_at");
  CREATE INDEX IF NOT EXISTS "product_attributes_created_at_idx" ON "product_attributes" USING btree ("created_at");
  ...
  CREATE INDEX IF NOT EXISTS "product_images_sizes_thumbnail_sizes_thumbnail_filename_idx" ON "product_images" USING btree ("sizes_thumbnail_filename");
  CREATE INDEX IF NOT EXISTS "product_images_sizes_card_sizes_card_filename_idx" ON "product_images" USING btree ("sizes_card_filename");
  CREATE INDEX IF NOT EXISTS "product_images_sizes_desktop_sizes_desktop_filename_idx" ON "product_images" USING btree ("sizes_desktop_filename");
  ...
  CREATE INDEX IF NOT EXISTS "payload_locked_documents_rels_product_tags_id_idx" ON "payload_locked_documents_rels" USING btree ("product_tags_id");
  ...
  CREATE INDEX IF NOT EXISTS "payload_locked_documents_rels_product_attributes_id_idx" ON "payload_locked_documents_rels" USING btree ("product_attributes_id");
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  DROP TABLE "product_attributes_values" CASCADE;
